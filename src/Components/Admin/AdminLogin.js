  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        console.log(decodedToken); // Print the decoded token to check its contents (role, email, etc.)
    ...
    <div className="flex justify-center min-h-screen font-sans">
      <div className="bg-white shadow-md rounded-lg w-full max-w-md p-6 h-full min-h-screen">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className='mt-3'>
            <label htmlFor="email" className="block text-[#8F959E] font-sans text-base mb-1">Email</label>
            <input
              id="email"
              type="email"
              {...register("email", {
          ...
          <div className="relative mt-3">
            <label htmlFor="password" className="block text-[#8F959E] font-sans text-base mb-1">Password</label>
            <input
              id="password"
              type="password"
              {...register("password", {
          ...
          <button
            type="submit"
            className={`font-sans w-full bg-[#5EB66E] text-white py-2 font-bold rounded-md hover:bg-green-700 transition duration-300 !mt-6 text-[16px] ${isLoading ? 'opacity-50' : ''}`}
            disabled={isLoading}
          >
